...
def create(request: schemas.Blog, db: Session, current_user_id: int):
...
def destroy(id: int, db: Session, current_user_id: int):
...
def update(id: int, request: schemas.Blog, db: Session, current_user_id: int):
    blog = db.query(models.Blog).filter(models.Blog.id == id)
    if not blog.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Blog with id {id} not found")
